...
<template>
	<n-modal v-model:show="localShow" title="详情参数" preset="card" size="huge" :mask-closable="false"
		:style="{ width: '1000px', height: '560px', paddingBottom: '20px', position: 'relative' }"
		:content-style="{ height: '100%', overflow: 'auto' }" @update:show="onClose">
		<div class="w-80% mx-auto py-4" ref="detailBox">
			<img class="block w-80% mx-auto mb-4 rounded-xl" :src="props.image">
			<div class="w-80% mx-auto grid grid-cols-2 gap-y-2 gap-x-1">
				<div class="flex-start" v-for="item in currentExif" :key="item?.name">
		...
		<n-popover trigger="hover" :show-arrow="false">
			<template #trigger>
				<n-button class="absolute bottom-10 right-10" strong secondary circle type="info"
