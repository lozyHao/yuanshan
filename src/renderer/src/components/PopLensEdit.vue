...
import { OptionLensValues, IconSize } from '@renderer/interfaces/options'
    ...
    <div class="pre-edit-pop">
      <n-grid x-gap="12" y-gap="16" :cols="6">
        ...
        <n-gi class="flex-start" :span="3" v-if="formValue.contentType == 1">
          <img
            class="h-10 w-50 object-contain"
            v-if="formValue.contentImage"
            :src="formValue.contentImage"
          />
          <n-space vertical align="end" class="w-full">
            <upload-file
              :iconSize="IconSize.Mini"
              @on-change="($event) => (formValue.contentImage = $event)"
            ></upload-file>
            <camera-select
              @on-choose="($event) => (formValue.contentImage = $event)"
            ></camera-select>
          </n-space>
        </n-gi>
        <!-- 文字样式在类型为文字时使用 -->
        ...
        <n-gi class="flex-start" :span="2">
          <n-switch
            size="small"
            v-model:value="formValue.italic"
            :disabled="!formValue.forcedUsed"
          ></n-switch>
        </n-gi>
        ...
        <n-gi class="flex-start" :span="5">
          ...
          <span
            class="color3 ml-2 text-xl"
            :style="{
              fontFamily: formValue.font || '',
              fontWeight: formValue.bold ? 'bold' : '',
              fontStyle: formValue.italic ? 'italic' : ''
            }"
          >
            远山
          </span>
        ...
        <n-gi :span="3">
          <n-popconfirm
            ...
            :disabled="formValue.type === 0 || !formValue.key"
          >
            <template #trigger>
              <n-button
                type="error"
