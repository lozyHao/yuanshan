...
const draw = async (params: any, image: ImageBitmap, width: number, height: number, callbackId: string
) => {
	const { basic, textList } = params

	// 判断是否有内容绘制
	const haveText = textList && (textList.left.length > 0 || textList.right.length > 0 || textList.center.length > 0)

	const CanvasDraw = await import('./canvasDraw.ts')
	const canvasDraw = new CanvasDraw.default(width, height, basic.pattern, basic.textBgColorUsed, basic.mainImgSize, haveText);
	...
	// 绘制主图
	canvasDraw.drawMainImage({ image, width, height, shadow: basic.shadowSize, ratio: basic.mainImgSize, radius: basic.roundedSize })
	...
	if (basic.watermarkUsed && basic.watermark) {
		canvasDraw.drawWatermark({ image: basic.watermark, size: basic.watermarkSize, position: basic.watermarkPosition })
	}
