import { OptionBasicValues, OptionTextTemplateValues, OptionLensValues, ImageTextItem, TextTemplatePositionEnum } from '@/interfaces/options'
import ExifFactory from "./exifFactory";
import { getImageBitmap } from '@/utils/tool'
...
}): Promise<{ basic: OptionBasicValues, textList: { [key: string]: { items: ImageTextItem[]; x: TextTemplatePositionEnum }[] } }> => {
	const { exif, basic, text, lens } = options

	const allList = text.filter(item => item.position !== 'no')
	const list: { items: ImageTextItem[]; x: TextTemplatePositionEnum }[] = await getTemplate(allList, lens, exif)
...
// 解析模板参数
const getTemplate = async (list: OptionTextTemplateValues[], lens: OptionLensValues[], exif: ExifFactory): Promise<{ items: ImageTextItem[]; x: TextTemplatePositionEnum }[] | []> => {
