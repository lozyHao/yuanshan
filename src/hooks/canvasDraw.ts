import { WatermarkPositionEnum } from "@/interfaces/options";
...
class CanvasDraw {
	...
	constructor(width: number, height: number, mode: Mode = 'out', border: boolean = false, ratio: number = 90, haveText: boolean = true) {
	...
	drawText(options: FileTextType) {
		const { text, font = 'Arial', x, y, color = "#000000", size = 40, bold = false, textAlign = "left", italic = false } = options
		this.ctx.save();
		this.ctx.fillStyle = color;
	...
	drawTextBackground(options: FillRectTextType) {
		const { color = "#ffffff", width = this.width, height = this.minSize * this.borderRatio, x = 0, y = this.height - this.minSize * this.borderRatio } = options
		if (this.mode === "in") {
			return
		}
	...
	drawMainImage(options: DrawMainImageType) {
		const { image, width = this.width, height = this.height, ratio = 90, shadow = 0.5, radius = 5 } = options
	...
	drawWatermark(options: WatermarkType) {
		...
		let y = (this.height - borderHeight) / 2 - h / 2
		switch (position) {
			case 'top':
				x = (this.width - w) / 2;
				y = (this.height - borderHeight) * ((1 - this.ratio / 100) / 2) + h / 2
				break;
			case 'bottom':
				x = (this.width - w) / 2;
				y = (this.height - borderHeight) * (this.ratio / 100) + (this.height - borderHeight) * ((1 - this.ratio / 100) / 2) - h * 3 / 2
				break;
			case 'leftTop':
				x = this.width * ((1 - this.ratio / 100) / 2) + h / 2;
				y = (this.height - borderHeight) * ((1 - this.ratio / 100) / 2) + h / 2
				break;
			case 'leftBottom':
				x = this.width * ((1 - this.ratio / 100) / 2) + h / 2;
				...
				break;
			case 'rightTop':
				x = this.width - this.width * ((1 - this.ratio / 100) / 2) - h * 3 / 2;
				y = (this.height - borderHeight) * ((1 - this.ratio / 100) / 2) + h / 2
				break;
			case 'rightBottom':
				x = this.width - this.width * ((1 - this.ratio / 100) / 2) - h * 3 / 2;
