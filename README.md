...
2. **参数实时预览**：调整任意参数后预览自动刷新（防抖），无需再手动点击按钮。
3. **预设模板**：内置 6 套常用风格模板，侧边栏「模板」标签一键套用，套用后可继续微调。
4. **配置导入 / 导出**：将基础配置、文字模板、相机参数整合为单个 JSON 文件，便于备份、迁移与分享（导出文件与预设模板同构）。
5. 报告生成导出、照片参数拷贝、自定义参数和模板。
6. 图片大小无限制，画质保留无压缩(可执行设置压缩百分比)。
7. 平台支持暗黑模式、恢复默认状态、文字转图片和画板等工具。
8. 安装时支持自定义安装路径。
9. 项目开源任意玩。
