...
5. 安装时支持自定义安装路径。
6. **参数实时预览**：调整任意参数后预览自动刷新（防抖），无需再手动点击按钮。
7. **预设模板**：内置 6 套常用风格模板，侧边栏「模板」标签一键套用，套用后可继续微调。
8. **配置导入 / 导出**：将基础配置、文字模板、相机参数整合为单个 JSON 文件，便于备份、迁移与分享（导出文件与预设模板同构）。
9. **相机自带参数编辑**：支持自带参数的编辑/新增。
10. **预览界面可拖拽 / 缩放**：支持预览图的拖拽/缩放，方便查看细节。
